...
export class MockSpanner {
  ...
  private static toPartialResultSetsWithChunkedValues(
    resultSet: protobuf.ResultSet,
    queryMode:
      | google.spanner.v1.ExecuteSqlRequest.QueryMode
      | keyof typeof google.spanner.v1.ExecuteSqlRequest.QueryMode,
    ...
    for (let i = 0; i < resultSet.rows.length; i += rowsPerPartialResultSet) {
      ...
      if (remainingChunk) {
        partial.values.push(remainingChunk);
        remainingChunk = undefined;
      }
      if (remainingValues) {
        partial.values.push(...remainingValues);
      ...
      for (
        ...
        if (row === resultSet.rows.length - 2) {
          partial.values.push(resultSet.rows[row].values![0]);
          const chunk1 = {
            stringValue: resultSet.rows[row].values![1].stringValue?.substring(
              0,
              4
            ),
          };
          partial.chunkedValue = true;
          partial.values.push(chunk1);

          remainingValues = [
            {
              stringValue: resultSet.rows[
                row
              ].values![1].stringValue?.substring(4, 7),
            },
          ];
          remainingValues.push(resultSet.rows[row].values![2]);
          remainingValues.push(resultSet.rows[row].values![3]);
        } else {
          partial.values.push(...resultSet.rows[row].values!);
        }
      }
