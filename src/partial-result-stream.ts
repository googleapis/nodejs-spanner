...
export class PartialResultStream extends Transform implements ResultEvents {
  ...
  private _fields!: google.spanner.v1.StructType.IField[];
  ...
  private _createRow(values: Value[]): Row {
    const fields = values.map((value, index) => {
      ...
      return {name: name || '', value: codec.decode(value, type!)};
    });
  ...
  static mergeLists(
    ...
    let listType: google.spanner.v1.IType;
