...
describe('Autogenerated Admin Clients', () => {
  ...
  // query_with_float32_parameter
  it('should use a FLOAT32 query parameter to query record from the Venues example table', async () => {
    const output = execSync(
      `${datatypesCmd} queryWithFloat32 ${INSTANCE_ID} ${DATABASE_ID} ${PROJECT_ID}`
    );
    assert.match(output, /VenueId: 4, VenueName: Venue 4, RatingScore: 0.9/);
    assert.match(output, /VenueId: 19, VenueName: Venue 19, RatingScore: 0.9/);
    assert.match(output, /VenueId: 42, VenueName: Venue 42, RatingScore: 0.9/);
  });
