...
const MULTI_REGION_INSTANCE_ID = 'test-samples-mr-instance-node';
...
describe('Autogenerated Admin Clients', () => {
  ...
  it('should restore database from a backup using an encryption key', async function () {
    ...
    const key = getCryptoKey(KEY_LOCATION_ID1);
  ...
  describe('encrypted database and backups with multiple KMS keys', () => {
    ...
    after(async () => {
      ...
      await Promise.all([
        instance.database(ENCRYPTED_DATABASE_ID).delete(GAX_OPTIONS),
        instance.backup(ENCRYPTED_BACKUP_ID).delete(GAX_OPTIONS),
        instance.backup(COPY_BACKUP_ID).delete(GAX_OPTIONS),
        instance.database(ENCRYPTED_RESTORE_DATABASE_ID).delete(GAX_OPTIONS),
      ]);
    ...
    it('should create a database with multiple KMS keys', async () => {
      ...
      assert.match(
        output,
        new RegExp(
          `Created database ${ENCRYPTED_DATABASE_ID} on instance ${MULTI_REGION_INSTANCE_ID}.`
        )
      );
    ...
    it('should copy backup with multiple KMS keys', async () => {
      const sourceBackupPath = `projects/${PROJECT_ID}/instances/${MULTI_REGION_INSTANCE_ID}/backups/${ENCRYPTED_BACKUP_ID}`;
      const output = execSync(
