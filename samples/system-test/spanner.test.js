...
<<<<<<< HEAD
    it('should create an example instance without default backup schedule type and update the instance to have it',
       async () => {
         const createInstanceOutput = execSync(`${createInstanceWithoutDefaultBackupSchedulesCommand} "${
             SAMPLE_INSTANCE_ID}" ${PROJECT_ID}`);
         assert.match(
             createInstanceOutput,
             new RegExp(`Created instance ${
                 SAMPLE_INSTANCE_ID} without default backup schedules.`));

         const updateInstanceOutput =
             execSync(`${updateInstanceDefaultBackupScheduleTypeCommand} "${
                 SAMPLE_INSTANCE_ID}" ${PROJECT_ID}`);
         assert.match(
             updateInstanceOutput,
             new RegExp(`Instance ${
                 SAMPLE_INSTANCE_ID} has been updated with the AUTOMATIC default backup schedule type.`));
       });
=======
...
>>>>>>> 4b65fec (chore(samples): Add samples for Cloud Spanner Default Backup Schedules)
