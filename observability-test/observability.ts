...
import {ContextManager, SpanStatusCode} from '@opentelemetry/api';
...
describe('getActiveOrNoopSpan', () => {
  let globalProvider: typeof NodeTracerProvider;
...
describe('Invoking methods creates spans', () => {
...
describe('Extended tracing', () => {
  ...
  beforeEach(async () => {
    ...
    await database.run('SELECT CURRENT_TIMESTAMP()');

    // Give the batch session creation, ample time to happen so
    // as not to pollute our tests with spans that were associated
    // with session creation, before creating the tracerProvider.
  ...
  it('By default or with extended tracing disabled', () => {
    const sopts = {sql: 'SELECT 1', opts: {enableExtendedTracing: false}};
  ...
  it('startTrace with enableExtendedTracing=true', () => {
    const opts = {sql: 'SELECT 1', opts: {enableExtendedTracing: true}};
...
describe('Capturing sessionPool annotations', () => {
  ...
  it('Check for annotations, cached session', async () => {
    ...
    const wantSpans = [
      'cloud.google.com/nodejs/spanner/Database.runStream',
      'cloud.google.com/nodejs/spanner/Database.run',
      'cloud.google.com/nodejs/spanner/Snapshot.runStream',
    ];
    ...
    assert.ok(
      gotEvents.length > mandatoryEvents.length,
      'number of events must be >= mandatory events'
    );

    mandatoryEvents.forEach(event => {
      assert.ok(gotEvents.includes(event), 'each event contains');
    });
  });
});
