...
## Spanner Application stuck or Session Leak Debugging

A Spanner object of the Client Library has a limit on the number of maximum sessions. When all the
sessions are checked out of the session pool, every new transaction has to wait until a session is 
returned to the pool. If a session is never returned to the pool(hence causing a session leak), 
the transactions will have to wait indefinitely and your application will be blocked. The most 
common reason for a session leak is that a transaction was started by the application but never
committed or rolled back. 

### Debugging Session Leaks
Session Pool options can be set to help identify transactions that might be causing session leaks.
#### Logging
The logging option warns users when they have exhausted >95% of the session pool associated with 
the client side database object. Stack traces of transaction which might be causing these leaks
will also be logged. These transactions can be analysed to find points which might be causing
the leak.
#### Automatically Clean Inactive Transactions
When this option is enabled, the client library will automatically spot problematic transactions 
that are running for extremely long periods of time that might be causing a leak and close them. 

#### Code Snippet to pass Session Pool options

```javascript
const spanner = new Spanner({projectId: projectId,});
const instance = spanner.instance(instanceId);

// pass session pool options with closeInactiveTransactions and // logging true
...
// set customized logger if required
const transports = [
  new winston.transports.File({
    filename: 'combined.log',
  }),
];
const format = winston.format.combine(winston.format.metadata());
